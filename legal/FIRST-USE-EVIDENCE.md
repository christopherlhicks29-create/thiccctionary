# First-Use Evidence — Thiccctionary

**Purpose:** Document evidence of Thiccctionary's first commercial use of the brand name "Thiccctionary." For use in USPTO trademark filing and any future opposition or enforcement action.

**Author:** Christopher L. Hicks (christopher.l.hicks29@gmail.com)
**Brand:** Thiccctionary (also: "The Thiccctionary," "thiccctionary.com")
**Compiled:** 2026-05-09

## The brand at a glance

- **Public-facing entity:** Thiccctionary, a satirical online daily dictionary documenting inanimate objects of unusual girth.
- **Editorial discipline:** Strictly things, never people. The brand's defining position.
- **Domain:** thiccctionary.com
- **Repository:** https://github.com/christopherlhicks29-create/thiccctionary (private until/unless made public for evidence)

## Evidence of first commercial use

### Domain registration

- **Domain:** thiccctionary.com
- **Registrar:** Cloudflare Registrar (transferred to / registered through Cloudflare)
- **Registration date:** [TODO: pull from Cloudflare dashboard. Likely April 2026.]
- **Public WHOIS:** ICANN lookup at https://lookup.icann.org/lookups/domain/thiccctionary.com

### Website launch

- **First public availability:** April 2026 (exact date from initial commit / Cloudflare Pages deploy log)
- **First entry published:** 2026-04-26 (Pumpkin, Atlantic Giant — see entries/2026-04-26.html)
- **Initial daily-cron deployment:** April 2026

### Git repository history

- **Repository:** christopherlhicks29-create/thiccctionary on GitHub
- **First commit:** [pull from `git log --reverse | head -5` — earliest commit is canonical first-use timestamp]
- **Authored by:** Christopher L. Hicks
- **All commits:** publicly verifiable in repo history once made public

### Social media accounts (controlled by brand)

| Platform | Handle | Linked from thiccctionary.com sameAs schema |
|----------|--------|--------------------------------------------|
| Instagram | @ogthiccctionary | Yes |
| Facebook | Thiccctionary (page) | Yes |
| X / Twitter | @thiccctionary | Yes |
| TikTok | @thethiccctionary | Yes (added 2026-05-09) |
| Buy Me A Coffee | Thiccctionary | Yes |

### Public-facing brand assets

- Press kit page: thiccctionary.com/press/ (live since [date])
- Foundational definition entry: thiccctionary.com/thiccc/ (live 2026-05-09)
- Articles directory with 10 long-form pieces under thiccctionary.com/articles/
- Legal pages: thiccctionary.com/legal/terms.html, thiccctionary.com/legal/privacy.html

### Schema.org Organization declarations

The site declares itself as the "Thiccctionary" Organization in its homepage JSON-LD schema, with comprehensive sameAs links to all official social accounts. This is publicly crawlable and dated.

## Known conflict

### Account: @thiccctionaryyy on TikTok

- **URL:** https://www.tiktok.com/@thiccctionaryyy
- **Display name:** "Thiccctionary" (identical to brand name)
- **Followers as of 2026-05-09:** 9,323
- **Total likes:** 178,000
- **Content type:** Personal selfie / body-focused content (not a dictionary, not satire of objects, no editorial reference work being produced)
- **Date observed:** 2026-05-09
- **Screenshot evidence:** Saved separately by Christopher; will be included in trademark attorney consultation packet.

### Why this matters

The display name "Thiccctionary" used by @thiccctionaryyy is identical to the brand name used by thiccctionary.com. The classes of goods/services differ:

- **thiccctionary.com:** A satirical online dictionary — Class 41 (Education / publishing) and/or Class 16 (Printed publications) when book launches in 2027
- **@thiccctionaryyy:** Personal social media content — much narrower or no goods/services class

The class differential means coexistence is possible under trademark law, but the identical-display-name presents brand-confusion risk that the trademark application should explicitly address.

## Action items for trademark filing

1. **Confirm domain registration date** — pull from Cloudflare account
2. **Pull first-commit date** from git log
3. **Identify proper Nice classification** for the trademark filing — at minimum Class 41 (Education and entertainment services). For book: Class 16 (Printed publications).
4. **Document first commercial use** — earliest date of public-facing access to thiccctionary.com
5. **Compile screenshots:**
   - Homepage as it exists today
   - At least one daily entry showing dictionary structure
   - The /thiccc/ foundational entry
   - The /press/ kit
   - The Organization schema (view source)
6. **Schedule consultation with trademark attorney** — recommend $700-1500 budget for filing + strategic guidance on the @thiccctionaryyy conflict.

## Why this file exists

The trademark attorney will ask for first-use evidence. Having this compiled in advance saves consultation time and ensures we don't forget important documentation. It also serves as a permanent record of the brand's history that can be referenced years from now.

This file should be updated whenever a new official social account is added, a new significant brand asset is launched, or a new conflict is observed.

## Auto-pulled facts (run on 2026-05-09)

**First git commit:** 2026-05-01T22:10:56-06:00 0f863e9 Initial Thiccctionary
**Most recent commit at file creation:** 2026-05-09T16:06:12+00:00 c889c6a Wave 32: Strengthen brand entity recognition vs Google auto-correct
**Total commits at file creation:** 179
