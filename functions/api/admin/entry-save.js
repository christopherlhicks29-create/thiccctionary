...
export async function onRequestPost({ request, env }) {
  ...
  if (!entry || typeof entry !== 'object') return new Response(JSON.stringify({ error: 'entry required' }), { status: 400, headers: { 'Content-Type': 'application/json' } });
  if (!entry.word || !Array.isArray(entry.definitions) || entry.definitions.length === 0) {
    return new Response(JSON.stringify({ error: 'entry needs word + at least one definition' }), { status: 400, headers: { 'Content-Type': 'application/json' } });
  }
  ...
  const entries = JSON.parse(new TextDecoder().decode(rawBytes));
    ...
    if (!putRes.ok) {
      ...
      return new Response(JSON.stringify({ error: `Commit failed: ${putRes.status} ${err.slice(0,200)}` }), { status: 502, headers: { 'Content-Type': 'application/json' } });
    }

    return new Response(JSON.stringify({ ok: true, date, word: entries[idx].word }), { status: 200, headers: { 'Content-Type': 'application/json' } });
  } catch (e) {
