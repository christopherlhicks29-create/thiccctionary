...
Fixed in commit ce80709 (2026-08-08, Wave 337): loosened to `minSubjectPct: 15`. Validated live by dispatching `fire-reel.yml` for 2026-08-08: the day's image (score=6, subjectPct=24, which failed the old 25 floor) now passes (`PASS (score=7, subj%=24)` -- score also crept up a point on re-run, critic scoring has some run-to-run variance) and IG accepted the post.

New follow-up from that same validation: Facebook rejected the post synchronously with "Invalid post: Video could not be read from its URL" against the jsDelivr-hosted MP4 (the CDN Wave 306 switched to specifically because Cloudflare blocks Meta's own fetcher on the main domain). IG, using the identical video URL, succeeded. Not yet diagnosed whether this is a one-off jsDelivr edge-cache miss or a recurring FB-specific problem -- next session should re-fire `fire-reel.yml` for a couple of dates and watch whether FB keeps failing on fresh jsDelivr URLs specifically, or whether a retry a few minutes later succeeds (which would point to propagation delay rather than a systemic issue).

Also updates the 08-04/08-06 catalog-gap item below: this session dispatched daily.yml backfills for both. 08-04 was cancelled mid-run by daily.yml's concurrency: cancel-in-progress setting because a second backfill (08-06) was dispatched too soon after -- confirmed via its own trace log. 08-06 ran to completion but gracefully bailed after exhausting its subject-queue attempts (Tuba/Briefcase-class rejections, not a crash). Neither date is backfilled yet. Next session: dispatch ONE daily.yml backfill at a time, wait for it to finish, before firing the next -- do not batch these like this session did.
